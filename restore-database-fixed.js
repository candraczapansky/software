...
async function restoreData() {
  ...
  try {
    ...
    // 8. Create Sample Appointments (without room_id since it doesn't exist in schema)
    ...
    for (const appointment of appointments) {
      const result = await sql`
        INSERT INTO appointments (client_id, service_id, staff_id, start_time, end_time, status, notes)
        VALUES (${appointment.clientId}, ${appointment.serviceId}, ${appointment.staffId}, ${appointment.startTime}, ${appointment.endTime}, ${appointment.status}, ${appointment.notes})
