...
(async () => {
  ...
  const anna = staff.find(s => `${s.user?.firstName || ''} ${s.user?.lastName || ''}`.trim().toLowerCase() === 'anna keller');
  if (!anna) throw new Error('Anna Keller not found');
  ...
  const desired = [
    ...
    { name: 'Employee Full Set / $60', rate: 0.40 },
    ...
    { name: 'Lash Lift & Tint / $85', rate: 0.40 },
  ...
  const existingAssignments = await http('GET', `/api/staff-services?staffId=${anna.id}`);
  ...
  const finalAssignments = await http('GET', `/api/staff-services?staffId=${anna.id}`);
  console.log(JSON.stringify({ staffId: anna.id, added: dedup.size, total: Array.isArray(finalAssignments)?finalAssignments.length:finalAssignments, missing }, null, 2));
})();
