...
async function checkSchema() {
  ...
  try {
    ...
    console.log('\n📍 Locations table:');
    const locationsColumns = await sql`
      SELECT column_name, data_type 
      FROM information_schema.columns 
      WHERE table_name = 'locations' 
      ORDER BY ordinal_position
    `;
    locationsColumns.forEach(col => console.log(`  ${col.column_name}: ${col.data_type}`));
    ...
    const roomsColumns = await sql`
      ...
      ORDER BY ordinal_position
    `;
    roomsColumns.forEach(col => console.log(`  ${col.column_name}: ${col.data_type}`));
